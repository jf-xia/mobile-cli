...
import { AndroidDeviceManager, AndroidRobot } from "./android.ts";
...
import { ActionableError, type Button, type Orientation, type ScreenElement, type ScreenSize, type SwipeDirection } from "./robot.ts";
...
export class AndroidAutomationService {
	...
	private getRobotFromDevice(deviceId: string): AndroidRobot {
		const manager = new AndroidDeviceManager();
		const device = manager.getConnectedDevices().find(candidate => candidate.deviceId === deviceId);
		if (!device) {
	...
	public doctor() {
		...
		return {
			...
			stateFile: process.env.ANDROID_AUTOMATION_RECORDING_STATE_FILE || process.env.ANDROID_AUTOMATION_STATE_FILE || path.join(os.tmpdir(), "android-automation-recordings.json"),
		};
	}

	public listAvailableDevices() {
		return {
			devices: new AndroidDeviceManager().getConnectedDevicesWithDetails().map(device => ({
				id: device.deviceId,
				name: device.name,
				platform: "android",
				type: device.deviceType === "tv" ? "emulator" : "emulator",
	...
	public allocateRemoteDevice(platform: "android" | "ios") {
		this.ensureMobilecliAvailable();
	...
	public async openUrl(device: string, url: string) {
		const allowUnsafeUrls = process.env.ANDROID_AUTOMATION_ALLOW_UNSAFE_URLS === "1";
		if (!allowUnsafeUrls && !url.startsWith("http://") && !url.startsWith("https://")) {
			throw new ActionableError("Only http:// and https:// URLs are allowed. Set ANDROID_AUTOMATION_ALLOW_UNSAFE_URLS=1 to allow other schemes.");
		}
	...
	public async saveScreenshot(device: string, output?: string) {
		const targetPath = output || path.join(os.tmpdir(), `android-screenshot-${Date.now()}.png`);
	...
	public async startScreenRecording(device: string, output?: string, timeLimit?: number) {
		...
		const outputPath = output || path.join(os.tmpdir(), `android-recording-${Date.now()}.mp4`);
